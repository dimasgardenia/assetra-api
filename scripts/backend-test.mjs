...
await t('register with "admin" in email does NOT grant admin role', async () => { const r = await req('POST', '/api/auth/register', { body: { email: 'admin.palsu@gmail.com', password: 'rahasia123', name: 'Palsu', accountType: 'buyer', phone: '081299998888' } }); const u = db.prepare('SELECT role FROM users WHERE email = ?').get('admin.palsu@gmail.com'); return { ok: [201, 503].includes(r.status) && u?.role === 'bidder', note: `status ${r.status}, role ${u?.role}` }; });
